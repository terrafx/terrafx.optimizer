...
try {
  ...
  if ($solution -eq "") {
    $solution = Join-Path -Path $RepoRoot -ChildPath "TerraFX.Optimizer.sln"
  }
  ...
  if ($architecture -ne "") {
    ...
    & $DotNetInstallScript -Channel release/3.0.1xx -Version 3.0.100-preview6-012264 -InstallDir $DotNetInstallDirectory -Architecture $architecture
