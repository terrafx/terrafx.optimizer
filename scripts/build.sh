...
if [[ -z "$solution" ]]; then
  solution="$RepoRoot/TerraFX.Optimizer.sln"
fi
...
if [[ ! -z "$architecture" ]]; then
  ...
  . "$DotNetInstallScript" --channel release/3.0.1xx --version 3.0.100-preview6-012264 --install-dir "$DotNetInstallDirectory" --architecture "$architecture"
